...
class DoubanMovie(Spider):
    ...
    def parse(self, response):
        if 200 == response.status:
            ...
            for dataItem in data:
                ...
                # 影片图片详情
                # picUrl = dataItem['url'] + 'photos?type=R'
                print ("影片Outer")
                print (dataItem)
                ...
                # yield Request(awardUrl, headers=self.headers, callback=self.parse_movie_pic)
    ...
    def parse_movie_rewards(self, response):
        if response.status == 200:
            ...
            for awardTypeTag in awardTypeTagList:
                ...
                for awardTag in awardTagList:
                    ...
                    for awardUserTag in awardUserTagList:
                        awardUser = {}
                        awardUser['id'] = ''.join(awardUserTag.xpath("./attribute::href").extract()).replace('https://movie.douban.com/celebrity/', '').replace('/', '')
            ...
            print ("影片Reward")
            print (movieReward)
            self.movieRewardList.append(movieReward)
        else:
            url = response.url
